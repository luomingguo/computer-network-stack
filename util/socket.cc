...
void Socket::connect(const Address &address) {
  CheckSystemCall("connect",
                  ::connect(fd_num(), address.raw(), address.size()));
}
...
void DatagramSocket::sendto(const Address &destination,
  ...
  CheckSystemCall("sendto", ::sendto(fd_num(), payload.data(), payload.length(),
                                     0, destination.raw(), destination.size()));
  register_write();
}
