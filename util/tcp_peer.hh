...
public:
  ...
  void push(const TransmitFunction &transmit) {
    sender_.push(make_send(transmit));
  }
  ...
  bool active() const {
    const bool any_errors =
        receiver_.reader().has_error() or sender_.writer().has_error();
    const bool sender_active = sender_.sequence_numbers_in_flight() or
                               not sender_.reader().is_finished();
    const bool receiver_active = not receiver_.writer().is_closed();
    const bool lingering =
        linger_after_streams_finish_ and
        (cumulative_time_ < time_of_last_receipt_ + 10UL * cfg_.rt_timeout);
  ...
  void receive(TCPMessage msg, const TransmitFunction &transmit) {
    ...
    // If SenderMessage is a "keep-alive" (with intentionally invalid seqno),
    // make sure to reply. (N.B. orthodox TCP rules require a reply on any
    // unacceptable segment.)
    const auto our_ackno = receiver_.send().ackno;
    need_send_ |=
        (our_ackno.has_value() and msg.sender.seqno + 1 == our_ackno.value());

    // Did the inbound stream finish before the outbound stream? If so, no need
    // to linger after streams finish.
...
private:
  ...
  void send(const TCPSenderMessage &sender_message,
  ...
  bool linger_after_streams_finish_{
      true}; // one peer may need to linger to make sure all closure conditions
             // met
