...
//! checking that the source and destination ports in the TCP header are
//! correct.
//!
//! If the TCP connection is listening (i.e.,
//! TCPOverIPv4OverTunFdAdapter::_listen is `true`) and the TCP segment read
//! from the wire includes a SYN, this function clears the
//! `_listen` flag and records the source and destination addresses and port
//! numbers from the TCP header; it uses this information to filter future
//! reads. \returns a std::optional<TCPSegment> that is empty if the segment was
//! invalid or unrelated
optional<TCPMessage>
TCPOverIPv4Adapter::unwrap_tcp_in_ip(const InternetDatagram &ip_dgram) {
...
//! Takes a TCP segment, sets port numbers as necessary, and wraps it in an IPv4
//! datagram \param[in] seg is the TCP segment to convert
InternetDatagram TCPOverIPv4Adapter::wrap_tcp_in_ip(const TCPMessage &msg) {
  ...
  ip_dgram.header.len = ip_dgram.header.hlen * 4 + 20 /* tcp header len */ +
                        seg.message.sender.payload.size();
