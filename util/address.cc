...
//! man3::getaddrinfo)
inline addrinfo make_hints(int ai_flags,
                           int ai_family) // NOLINT(*-swappable-parameters)
{
...
pair<string, uint16_t> Address::ip_port() const {
  if (_address.storage.ss_family != AF_INET and
      _address.storage.ss_family != AF_INET6) {
...
string Address::to_string() const {
  if (_address.storage.ss_family == AF_INET or
      _address.storage.ss_family == AF_INET6) {
