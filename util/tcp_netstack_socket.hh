...
public:
  ...
  //! Connect using the specified configurations; blocks until connect succeeds
  //! or fails
  void connect(const TCPConfig &c_tcp, const FdAdapterConfig &c_ad);

  //! Listen and accept using the specified configurations; blocks until accept
  //! succeeds or fails
  ...
  const Address &peer_address() const {
    return _datagram_adapter.config().destination;
  }
...
private:
  ...
  //! eventloop that handles all the events (new inbound datagram, new outbound
  //! bytes, new inbound bytes)
