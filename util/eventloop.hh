...
private:
  ...
  struct FDRule : public BasicRule {
    ...
    CallbackT cancel; //!< A callback that is called when the rule is cancelled
                      //!< (e.g. on EOF or hangup)
    CallbackT error;  //!< A callback that is called when the fd has an error
                      //!< before cancellation

    FDRule(BasicRule &&base, FileDescriptor &&s_fd, Direction s_direction,
           CallbackT s_cancel, CallbackT s_error);
...
public:
  ...
  RuleHandle add_rule(
      ...
      const CallbackT &cancel = [] {}, const CallbackT &error = [] {});
