...
private:
  ...
  //! \param[in] uplink is `true` to use the uplink loss probability, else use
  //! the downlink loss probability \returns `true` if the segment should be
  //! dropped
...
public:
  ...
  //! \brief Read from the underlying AdapterT instance, potentially dropping
  //! the read datagram \returns std::optional<TCPSegment> that is empty if the
  //! segment was dropped or if
  ...
  //! \brief Write to the underlying AdapterT instance, potentially dropping the
  //! datagram to be written \param[in] seg is the packet to either write or
  //! drop
