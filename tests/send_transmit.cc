  ...
  try {
...

    {
      ...
      for (uint32_t i = 0; i < n_rounds; ++i) {
        ...
        const uint32_t block_size =
            uniform_int_distribution<uint32_t>{1, max_block_size}(rd);
        ...
        test.execute(
            ExpectMessage{}
                .with_seqno(isn + 1 +
                            static_cast<uint32_t>(bytes_sent - block_size))
                .with_data(move(data)));
        test.execute(ExpectNoSegment{});
        test.execute(
            AckReceived{Wrap32{isn + 1 + static_cast<uint32_t>(bytes_sent)}});
      }
    }

    {
      ...
      for (uint32_t i = 0; i < n_rounds; ++i) {
        ...
        test.execute(
            ExpectSeqno{Wrap32{isn + static_cast<uint32_t>(bytes_sent) + 1}});
