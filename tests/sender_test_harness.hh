  ...
  Wrap32 value(SenderAndOutput &ss) const override {
    ...
    if (seg.sequence_length()) {
      throw ExpectationViolation(
          "TCPSender::make_empty_message() returned non-empty message");
    }
    return seg.seqno;
  }
};
...
struct ExpectReset : public ExpectBool<SenderAndOutput> {
  ...
  bool value(SenderAndOutput &ss) const override {
    return ss.sender.make_empty_message().RST;
  }
};
...
struct ExpectSeqnosInFlight : public ExpectNumber<SenderAndOutput, uint64_t> {
  ...
  uint64_t value(SenderAndOutput &ss) const override {
    return ss.sender.sequence_numbers_in_flight();
  }
};

struct ExpectConsecutiveRetransmissions
    : public ExpectNumber<SenderAndOutput, uint64_t> {
  using ExpectNumber::ExpectNumber;
  std::string name() const override { return "consecutive_retransmissions"; }
  uint64_t value(SenderAndOutput &ss) const override {
    return ss.sender.consecutive_retransmissions();
  }
};
...
struct ExpectNoSegment : public Expectation<SenderAndOutput> {
  ...
  void execute(SenderAndOutput &ss) const override {
    if (not ss.output.empty()) {
      throw ExpectationViolation{"TCPSender sent an unexpected segment: " +
                                 to_string(ss.output.front())};
    }
  }
};
...
struct HasError : public ExpectBool<SenderAndOutput> {
  ...
  bool value(SenderAndOutput &ss) const override {
    return ss.sender.writer().has_error();
  }
};
...
struct Push : public Action<SenderAndOutput> {
  ...
  std::string description() const override {
    ...
    return "push \"" + Printer::prettify(data_) + "\" to stream" +
           (close_ ? ", close it" : "") + ", then push to TCPSender";
  }
  void execute(SenderAndOutput &ss) const override {
    if (not data_.empty()) {
...
struct Tick : public Action<SenderAndOutput> {
  ...
  void execute(SenderAndOutput &ss) const override {
    ...
    if (max_retx_exceeded_.has_value() and
        max_retx_exceeded_ != (ss.sender.consecutive_retransmissions() >
                               TCPConfig::MAX_RETX_ATTEMPTS)) {
      std::ostringstream desc;
      desc << "after " << ms_
           << " ms passed the TCP Sender "
...
struct Receive : public Action<SenderAndOutput> {
  ...
  std::string description() const override {
    std::ostringstream desc;
    desc << "receive(ack=" << to_string(msg_.ackno)
         << ", win=" << msg_.window_size << ")";
    if (push_) {
...
struct ExpectMessage : public Expectation<SenderAndOutput> {
  ...
  ExpectMessage &with_seqno(uint32_t seqno_) {
    return with_seqno(Wrap32{seqno_});
  }
  ...
  std::string description() const override {
    return "message sent with" + message_description();
  }

  void execute(SenderAndOutput &ss) const override {
    if (payload_size.has_value() and data.has_value() and
        payload_size.value() != data.value().size()) {
    ...
    if (payload_size.has_value() and
      ...
      throw ExpectationViolation("payload_size", payload_size.value(),
                                 seg.payload.size());
    }
    if (seg.payload.size() > TCPConfig::MAX_PAYLOAD_SIZE) {
      throw ExpectationViolation("payload has length (" +
                                 std::to_string(seg.payload.size()) +
                                 ") greater than the maximum");
    }
    if (data.has_value() and
        data.value() != static_cast<std::string>(seg.payload)) {
      throw ExpectationViolation(
          "Expecting payload of \"" + Printer::prettify(data.value()) +
...
public:
  TCPSenderTestHarness(std::string name, TCPConfig config)
      : TestHarness(move(name),
                    ...
                    {TCPSender{ByteStream{config.send_capacity}, config.isn,
                               config.rt_timeout}}) {}
};
