...
int main() {
  try {
    ...
    {
      const uint32_t isn =
          uniform_int_distribution<uint32_t>{0, UINT32_MAX}(rd);
    ...
    /* segment with SYN + data */
    {
      ...
      test.execute(HasAckno{false});
      test.execute(SegmentArrives{}.with_syn().with_seqno(isn).with_data(
          "Hello, CS144!"));
    ...
    /* segment with FIN (but can't be assembled yet) */
    {
      const uint32_t isn =
          uniform_int_distribution<uint32_t>{0, UINT32_MAX}(rd);
      TCPReceiverTestHarness test{
          "segment with FIN (but can't be assembled yet)", 4000};
      test.execute(HasAckno{false});
      test.execute(SegmentArrives{}.with_syn().with_seqno(isn));
      test.execute(SegmentArrives{}
                       .with_fin()
...

    {
      ...
      TCPReceiverTestHarness test{"missing first byte in the first segment",
                                  cap};
      test.execute(SegmentArrives{}.with_syn().with_seqno(isn));
...

    {
      ...
      TCPReceiverTestHarness test{
          "pushing bytes in reverse order in initial SYN", cap};
      test.execute(SegmentArrives{}.with_syn().with_seqno(isn));
      ...
      test.execute(SegmentArrives{}.with_seqno(isn + 1).with_data(
          std::string(1, bytes[cap - 1])));
      test.execute(BytesPushed{bytes.size()});
      test.execute(ExpectWindow{0});
      test.execute(
          ExpectAckno{Wrap32{static_cast<uint32_t>(isn + bytes.size() + 1)}});
      test.execute(ReadAll{"abcdefghij"});
    }
    ...
    {
      // The spec is more restrictive on RST acceptance; we use simpler logic in
      // CS144.
      TCPReceiverTestHarness test{"RST flag set -> stream error", 10};
      test.execute(
          SegmentArrives{}.with_seqno(rd()).with_rst().without_ackno());
...

    {
      ...
      test.execute(ReadAll("")); // tests if the code double-pushes segments
    ...
    // test credit: Tanmay Garg and Agam Mohan Singh Bhatia
    {
      ReassemblerTestHarness test{
          "insert last fully beyond capacity + empty string is last", 2};
