...
void do_test_1(const TCPConfig &cfg, default_random_engine &rd) {
  ...
  TCPReceiverTestHarness test_1{"non-overlapping out-of-order segments",
                                cfg.recv_capacity};
  ...
  for (auto [off, sz] : seq_size) {
    test_1.execute(SegmentArrives{}
                       .with_seqno(rx_isn + 1 + off)
                       .with_data(d.substr(off, sz)));
    if (off + 1 == min_expect_ackno) {
      min_expect_ackno = min_expect_ackno + sz;
    }
    max_expect_ackno = max_expect_ackno + sz;
    test_1.execute(
        ExpectAcknoBetween{rx_isn, off, min_expect_ackno, max_expect_ackno});
  }
...
void do_test_2(const TCPConfig &cfg, default_random_engine &rd) {
  const Wrap32 rx_isn(rd());
  TCPReceiverTestHarness test_2{"overlapping out-of-order segments",
                                cfg.recv_capacity};
  ...
  while (datalen < cfg.recv_capacity) {
    ...
    if (rem == 0) {
      ...
      offs =
          min(min(datalen, rem), 1 + (static_cast<size_t>(rd()) % (rem - 1)));
    }
