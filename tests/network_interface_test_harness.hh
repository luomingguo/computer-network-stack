  ...
  NetworkInterfaceTestHarness(std::string test_name,
      ...
      : TestHarness(
            ...
            [&] {
              ...
              const NetworkInterface iface{"test", output, ethernet_address,
                                           ip_address};
...
struct SendDatagram : public Action<InterfaceAndOutput> {
  ...
  void execute(InterfaceAndOutput &interface) const override {
    interface.first.send_datagram(dgram, next_hop);
  }

  SendDatagram(InternetDatagram d, Address n)
      : dgram(std::move(d)), next_hop(n) {}
};
...
struct ReceiveFrame : public Action<InterfaceAndOutput> {
  ...
  std::string description() const override {
    return "frame arrives (" + summary(frame) + ")";
  }
  void execute(InterfaceAndOutput &interface) const override {
    ...
    if (not expected.has_value()) {
      ...
      throw ExpectationViolation(
          "an arriving Ethernet frame was passed up the stack as an Internet "
          "datagram, but was not expected to be "
          "(did destination address match our interface?)");
    }

    if (inbound.empty()) {
      throw ExpectationViolation(
          "an arriving Ethernet frame was expected to be passed up the stack "
          "as an Internet datagram, but wasn't");
    }

    if (not equal(inbound.front(), expected.value())) {
      throw ExpectationViolation(
          std::string("NetworkInterface::recv_frame() produced a different "
                      "Internet datagram than was expected: ") +
...
struct ExpectFrame : public Expectation<InterfaceAndOutput> {
  ...
  void execute(InterfaceAndOutput &interface) const override {
    if (interface.second->frames.empty()) {
      throw ExpectationViolation("NetworkInterface was expected to send an "
                                 "Ethernet frame, but did not");
    }
    ...
    if (not equal(frame, expected)) {
      throw ExpectationViolation("NetworkInterface sent a different Ethernet "
                                 "frame than was expected: actual={" +
                                 summary(frame) + "}");
    }
  }
...
struct Tick : public Action<InterfaceAndOutput> {
  ...
  std::string description() const override {
    return to_string(_ms) + " ms pass";
  }
  void execute(InterfaceAndOutput &interface) const override {
    interface.first.tick(_ms);
  }
