...
public:
  void transmit(const NetworkInterface &sender,
                const EthernetFrame &frame) override {
    ranges::for_each(connections_, [&](auto &weak_ref) {
      const shared_ptr<NetworkInterface> interface(weak_ref);
      if (&sender != interface.get()) {
        cerr << "Transferring frame from " << sender.name() << " to "
             << interface->name() << ": " << summary(frame) << "\n";
        interface->recv_frame(frame);
      }
    });
  }

  void connect(const shared_ptr<NetworkInterface> &interface) {
    connections_.push_back(interface);
  }
};
...
class Host {
  ...
  void remove_expectation(const InternetDatagram &expected) {
    for (auto it = _expecting_to_receive.begin();
         it != _expecting_to_receive.end(); ++it) {
...
public:
  ...
  Host(string name, const Address &my_address, const Address &next_hop,
       shared_ptr<NetworkSegment> network)
      : _name(std::move(name)), _my_address(my_address),
        _interface(make_shared<NetworkInterface>(
            _name, move(network), random_host_ethernet_address(), _my_address)),
        _next_hop(next_hop) {}
  ...
  InternetDatagram send_to(const Address &destination, const uint8_t ttl = 64) {
    ...
    dgram.payload.emplace_back(
        string{"Cardinal " + to_string(random_device()() % 1000)});
    dgram.header.len = static_cast<uint64_t>(dgram.header.hlen) * 4 +
                       dgram.payload.back().size();
    dgram.header.ttl = ttl;
    dgram.header.compute_checksum();

    cerr << "Host " << _name
         << " trying to send datagram (with next hop = " << _next_hop.ip()
  ...
  void expect(const InternetDatagram &expected) {
    _expecting_to_receive.push_back(expected);
  }
    ...
    while (not _interface->datagrams_received().empty()) {
      ...
      if (not expecting(dgram_received)) {
        throw runtime_error("Host " + _name +
                            " received unexpected Internet datagram: " +
                            dgram_received.header.to_string());
      }
...
private:
  ...
  shared_ptr<NetworkSegment> upstream{make_shared<NetworkSegment>()},
      eth0_applesauce{make_shared<NetworkSegment>()},
      eth2_cherrypie{make_shared<NetworkSegment>()},
...
public:
  Network()
      : default_id(_router.add_interface(make_shared<NetworkInterface>(
            "default", upstream, random_router_ethernet_address(),
            Address{"171.67.76.46"}))),
        eth0_id(_router.add_interface(make_shared<NetworkInterface>(
            "eth0", eth0_applesauce, random_router_ethernet_address(),
            Address{"10.0.0.1"}))),
        eth1_id(_router.add_interface(make_shared<NetworkInterface>(
            "eth1", empty, random_router_ethernet_address(),
            Address{"172.16.0.1"}))),
        eth2_id(_router.add_interface(make_shared<NetworkInterface>(
            "eth2", eth2_cherrypie, random_router_ethernet_address(),
            Address{"192.168.0.1"}))),
        uun3_id(_router.add_interface(make_shared<NetworkInterface>(
            "uun3", uun, random_router_ethernet_address(),
            Address{"198.178.229.1"}))),
        hs4_id(_router.add_interface(make_shared<NetworkInterface>(
            "hs4", hs, random_router_ethernet_address(),
            Address{"143.195.0.2"}))),
        mit5_id(_router.add_interface(make_shared<NetworkInterface>(
            "mit5", empty, random_router_ethernet_address(),
            Address{"128.30.76.255"}))) {
    _hosts.insert({"applesauce",
                   {"applesauce", Address{"10.0.0.2"}, Address{"10.0.0.1"},
                    eth0_applesauce}});
    _hosts.insert(
        {"default_router",
         {"default_router", Address{"171.67.76.1"}, Address{"0"}, upstream}});
    _hosts.insert({"cherrypie",
                   {"cherrypie", Address{"192.168.0.2"}, Address{"192.168.0.1"},
                    eth2_cherrypie}});
    _hosts.insert(
        {"hs_router", {"hs_router", Address{"143.195.0.1"}, Address{"0"}, hs}});
    _hosts.insert(
        {"dm42",
         {"dm42", Address{"198.178.229.42"}, Address{"198.178.229.1"}, uun}});
    _hosts.insert(
        {"dm43",
         {"dm43", Address{"198.178.229.43"}, Address{"198.178.229.1"}, uun}});
    ...
    _router.add_route(ip("0.0.0.0"), 0, host("default_router").address(),
                      default_id);
    ...
    _router.add_route(ip("143.195.0.0"), 17, host("hs_router").address(),
                      hs4_id);
    _router.add_route(ip("143.195.128.0"), 18, host("hs_router").address(),
                      hs4_id);
    _router.add_route(ip("143.195.192.0"), 19, host("hs_router").address(),
                      hs4_id);
...
void network_simulator() {
  ...
  cout << green
       << "\n\nTesting traffic between two ordinary hosts (applesauce to "
          "cherrypie)..."
       << normal << "\n\n";
  {
    auto dgram_sent =
        network.host("applesauce").send_to(network.host("cherrypie").address());
  ...
  cout << green
       << "\n\nTesting traffic between two ordinary hosts (cherrypie to "
          "applesauce)..."
       << normal << "\n\n";
  {
    auto dgram_sent =
        network.host("cherrypie").send_to(network.host("applesauce").address());
    dgram_sent.header.ttl--;
    dgram_sent.header.compute_checksum();
  ...
  {
    auto dgram_sent =
        network.host("applesauce").send_to(Address{"143.195.131.17"});
    dgram_sent.header.ttl--;
    dgram_sent.header.compute_checksum();
  ...
  {
    auto dgram_sent =
        network.host("dm42").send_to(network.host("dm43").address());
    dgram_sent.header.ttl--;
    dgram_sent.header.compute_checksum();
  ...
  cout << "\n\n\033[32;1mCongratulations! All datagrams were routed "
          "successfully.\033[m\n";
}
