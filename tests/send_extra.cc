  ...
  try {
...

    {
      ...
      TCPSenderTestHarness test{
          "If already running, timer stays running when new segment sent", cfg};
      test.execute(Push{});
      ...
      test.execute(
          ExpectMessage{}.with_payload_size(3).with_data("abc").with_seqno(isn +
                                                                           1));
...

    {
      ...
      test.execute(Push("def"));
      test.execute(
          ExpectMessage{}.with_payload_size(3).with_data("def").with_seqno(isn +
                                                                           4));
...

    {
      ...
      TCPSenderTestHarness test{"Timer doesn't restart without ACK of new data",
                                cfg};
      test.execute(Push{});
      test.execute(ExpectMessage{}
                       .with_no_flags()
...

    {
      ...
      test.execute(
          ExpectMessage{}.with_payload_size(3).with_data("ghi").with_seqno(isn +
                                                                           7));
      test.execute(AckReceived{Wrap32{isn + 1}}.with_win(1000));
      test.execute(Tick{6});
      test.execute(
    ...
    }
    {
      ...
      const size_t window_size =
          uniform_int_distribution<uint16_t>{50000, 63000}(rd);

      TCPSenderTestHarness test{"fill_window() correctly fills a big window",
                                cfg};
      test.execute(Push{});
      test.execute(ExpectMessage{}
                       .with_no_flags()
...

    {
      ...
      TCPSenderTestHarness test{
          "Retransmit a FIN-containing segment same as any other", cfg};
      test.execute(Push{});
      test.execute(ExpectMessage{}
                       .with_no_flags()
                       .with_syn(true)
...

    {
      ...
      TCPSenderTestHarness test{
          "Retransmit a FIN-only segment same as any other", cfg};
      test.execute(Push{});
      test.execute(ExpectMessage{}
                       .with_no_flags()
                       .with_syn(true)
      ...
      test.execute(Close{});
      test.execute(
          ExpectMessage{}.with_payload_size(0).with_seqno(isn + 4).with_fin(
              true));
      test.execute(Tick{rto - 1});
      test.execute(ExpectNoSegment{});
...

    {
      ...
      TCPSenderTestHarness test{"Don't add FIN if this would make the segment "
                                "exceed the receiver's window",
                                cfg};
      test.execute(Push{});
      test.execute(ExpectMessage{}
                       .with_no_flags()
...

    {
      ...
      TCPSenderTestHarness test{
          "Don't send FIN by itself if the window is full", cfg};
      test.execute(Push{});
      test.execute(ExpectMessage{}
                       .with_no_flags()
                       .with_syn(true)
...

    {
      ...
      TCPSenderTestHarness test{"When filling window, treat a '0' window size "
                                "as equal to '1' but don't back off RTO",
                                cfg};
      test.execute(Push{});
      test.execute(ExpectMessage{}
                       .with_no_flags()
...

    {
      ...
      TCPSenderTestHarness test{"Unlike a zero-size window, a full window of "
                                "nonzero size should be respected",
                                cfg};
      test.execute(Push{});
      test.execute(ExpectMessage{}
                       .with_no_flags()
...

    {
      ...
      TCPSenderTestHarness test{"Repeated ACKs and outdated ACKs are harmless",
                                cfg};
      test.execute(Push{});
      test.execute(ExpectMessage{}
                       .with_no_flags()
      ...
      test.execute(
          ExpectMessage{}.with_payload_size(7).with_data("abcdefg").with_seqno(
              isn + 1));
    ...
    // Sender must set RST iff stream has suffered an error
    {
      ...
      const Wrap32 isn(rd());
      cfg.isn = isn;
      TCPSenderTestHarness test{"Stream error -> RST flag (non-empty segment)",
                                cfg};
      test.execute(Push{});
      test.execute(ExpectMessage{}.with_no_flags().with_syn(true));
      ...
      test.execute(
          ExpectMessage{}.with_no_flags().with_rst(true).with_seqno(isn + 1));
    }
    ...
    // Newly added test: check RTO timer on invalid ackno
    {
      ...
      cfg.isn = isn;
      cfg.rt_timeout = 10;
      TCPSenderTestHarness test{"invalid ackno -> should not reset RTO timer",
                                cfg};
      test.execute(Push{});
      test.execute(ExpectMessage{}
                       .with_no_flags()
      ...
      test.execute(ExpectNoSegment{});
      test.execute(Tick{10}.with_max_retx_exceeded(
          false)); // RTO timer should double here
      test.execute(ExpectMessage{}.with_payload_size(4));
      test.execute(ExpectNoSegment{});
      test.execute(AckReceived{
          Wrap32{isn + 1000}}); // invalid ackno received here, should not reset
      test.execute(Tick{10}.with_max_retx_exceeded(
          false)); // RTO timer should still be double
      test.execute(ExpectNoSegment{});
    }
    ...
    // test credit: Ava Jih-Schiff
    {
      ...
      test.execute(AckReceived{Wrap32{cfg.isn}}
                       .with_win(3)
                       .without_push()); // invalid ack but window is set
      test.execute(ExpectNoSegment{});
      test.execute(Push{"abc"}.with_close());
      test.execute(ExpectMessage{}
                       .with_no_flags()
      ...
      test.execute(AckReceived{Wrap32{isn + 3}}.with_win(1));
      test.execute(
          ExpectMessage{}.with_no_flags().with_payload_size(1).with_seqno(isn +
                                                                          3));
      test.execute(AckReceived{Wrap32{isn + 4}}.with_win(1));
      test.execute(ExpectMessage{}
                       .with_no_flags()
