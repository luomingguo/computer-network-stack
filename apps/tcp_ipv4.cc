...
void show_usage(const char *argv0, const char *msg) {
  cout << "Usage: " << argv0 << " [options] <host> <port>\n\n"
       ...
       << "   -Lu <loss>      Set uplink loss to <rate> (float in 0..1)       "
          "(no loss)\n"
       << "   -Ld <loss>      Set downlink loss to <rate> (float in 0..1)     "
          "(no loss)\n\n"
...
tuple<TCPConfig, FdAdapterConfig, bool, const char *>
...
int main(int argc, char **argv) {
  try {
    ...
    bidirectional_stream_copy(tcp_socket,
                              tcp_socket.peer_address().to_string());
