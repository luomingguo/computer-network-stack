...
inline std::pair<FileDescriptor, FileDescriptor> make_socket_pair() {
  ...
  CheckSystemCall("socketpair",
                  ::socketpair(AF_UNIX, SOCK_DGRAM, 0, fds.data()));
...
private:
  struct Sender : public NetworkInterface::OutputPort {
    ...
    void transmit(const NetworkInterface &n [[maybe_unused]],
                  const EthernetFrame &x) override {
...
public:
  NetworkInterfaceAdapter(const Address &ip_address,
                          const Address &next_hop) // NOLINT(*-swappable-*)
      : _interface("network interface adapter", sender_,
                   random_host_ethernet_address(), ip_address),
        _next_hop(next_hop) {}
  ...
  optional<TCPMessage> read() {
    ...
    // Give the frame to the NetworkInterface. Get back an Internet datagram if
    // frame was carrying one.
  ...
  void write(const TCPMessage &msg) {
    _interface.send_datagram(wrap_tcp_in_ip(msg), _next_hop);
  }
...
void program_body(bool is_client, const string &bounce_host,
                  const string &bounce_port, const bool debug) {
  ...
  if (is_client) {
    host_side = router.add_interface(make_shared<NetworkInterface>(
        "host_side", router_to_host, random_router_ethernet_address(),
        Address{"192.168.0.1"}));
    internet_side = router.add_interface(make_shared<NetworkInterface>(
        "internet side", router_to_internet, random_router_ethernet_address(),
        Address{"10.0.0.192"}));
    ...
    router.add_route(Address{"172.16.0.0"}.ipv4_numeric(), 12,
                     Address{"10.0.0.172"}, internet_side);
  } else {
    host_side = router.add_interface(make_shared<NetworkInterface>(
        "host_side", router_to_host, random_router_ethernet_address(),
        Address{"172.16.0.1"}));
    ...
    router.add_route(Address{"192.168.0.0"}.ipv4_numeric(), 16,
                     Address{"10.0.0.192"}, internet_side);
  }

  /* set up the client */
  TCPSocketEndToEnd sock =
      is_client
  ...
  thread network_thread([&]() {
    try {
      ...
      event_loop.add_rule(
          "frames from host to router", sock.adapter().frame_fd(),
          Direction::In, [&] {
      ...
      event_loop.add_rule(
          "frames from router to host", sock.adapter().frame_fd(),
          Direction::Out,
          [&] {
      ...
      event_loop.add_rule(
          "frames from Internet to router", internet_socket, Direction::In,
          [&] {
