...
void Reassembler::insert(uint64_t first_index, string data,
  ...
  if (first_index < out_index) {
    data = data.substr(out_index - first_index,
                       data.size() - (out_index - first_index));
  ...
  auto q = p;
  ++q;
  while (q != unassemble_subs_.end() &&
         q->first + q->second.size() <= p->first + p->second.size()) {
    pending_ -= q->second.size();
    q = unassemble_subs_.erase(q);
  }
  while (q != unassemble_subs_.end() &&
         q->first <= p->first + p->second.size()) {
